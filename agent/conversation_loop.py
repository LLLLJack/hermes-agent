...
from agent.turn_api_call import codex_quota_guard, handle_api_interrupt, nous_rate_limit_guard, perform_api_call
...
def _run_api_retry_loop(agent, s: _LoopState) -> Optional[Dict[str, Any]]:
    ...
    while s.retry_count < s.max_retries:
        _cq = _run_phase(codex_quota_guard, agent, s)
        if _cq.action == "return":
            return _cq.result
        if _cq.action == "break":
            return None
