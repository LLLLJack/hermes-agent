...
def get_vertex_credentials(
    ...
    """Return (fresh access_token, project_id) or (None, None).

    Credentials objects are cached per file content, but an upstream 401 must be
    able to force a new access token even when google-auth still considers the
    cached token unexpired.
    """
    if google is None:
